...
namespace AT1._1_Diploma
{
	public partial class LinkedLists : Form
	{
		...
		public static void BubbleSort(LinkedList<string> myCarCollection)
		{ int i, j;

			for (j = 0; j < myCarCollection.Count; j++)
			{
				for (i = 0; i < myCarCollection.Count; i++)
				{
				if (myCarCollection.ElementAt(j).CompareTo(myCarCollection.ElementAt(j + 1)) > 0)
					{
						LinkedListNode<string> curr = myCarCollection.Find(myCarCollection.ElementAt(j));
		...
		private void SortBtn_Click(object sender, EventArgs e)
		{
			...
			for (j = 0; j < myCarCollection.Count; j++)
			{
				for (i = 0; i < myCarCollection.Count -1; i++)
				{
					if (myCarCollection.ElementAt(i).CompareTo(myCarCollection.ElementAt(i + 1)) > 0)
					{
						LinkedListNode<string> curr = myCarCollection.Find(myCarCollection.ElementAt(i));
						var temp = curr.Next.Value;
			...
			i = 1;
			listBoxOutput.Items.Clear();
			foreach (var item in myCarCollection)
			{
